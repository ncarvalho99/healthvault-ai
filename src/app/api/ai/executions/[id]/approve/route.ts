...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (execution.status !== "PENDING_APPROVAL") {
      return NextResponse.json({ error: `Execution is already in '${execution.status}' state.` }, { status: 400 });
    }
    ...
    if (meta?.expiresAt) {
      ...
      if (isExpired) {
        ...
        return NextResponse.json(
          {
            error: "Esta proposta de alteração expirou. Solicite ao assistente uma nova proposta atualizada.",
    ...
    if (meta?.entityVersionAtProposal !== undefined && meta?.entityId) {
      if (meta.entityType === "medication") {
        ...
        if (currentVersion !== undefined && currentVersion !== meta.entityVersionAtProposal) {
          return NextResponse.json(
            {
              error: `Conflito de versão: este medicamento foi alterado (v${meta.entityVersionAtProposal} → v${currentVersion}) após a criação da proposta. Gere uma nova proposta.`,
        ...
        if (currentDiet && currentDiet.currentVersion !== meta.entityVersionAtProposal) {
          return NextResponse.json(
            {
              error: `Conflito de versão: o plano alimentar mudou (v${meta.entityVersionAtProposal} → v${currentDiet.currentVersion}) desde a geração desta sugestão.`,
              code: "VERSION_CONFLICT",
            },
            { status: 409 }
          );
        ...
        if (currentRec && currentRec.currentVersion !== meta.entityVersionAtProposal) {
          return NextResponse.json(
            {
              error: "Conflito de versão: a recomendação clínica foi alterada por outra operação.",
              code: "VERSION_CONFLICT",
            },
            { status: 409 }
          );
